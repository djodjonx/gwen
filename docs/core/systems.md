...
By default, `api.services.get(...)` returns `unknown`. To get full autocompletion, annotate the `api` parameter with `EngineAPI<GwenServices>`.

**`GwenServices` is a global type** — automatically generated by `gwen prepare` into `.gwen/gwen.d.ts`. It is inferred from your `gwen.config.ts` and available **everywhere in your project without any import**.

```typescript
// ❌ Without type annotation — services are unknown
export const PlayerSystem = defineSystem({
  name: 'PlayerSystem',
  onUpdate(api, dt) {
    const kb = api.services.get('keyboard'); // unknown ⚠️
  }
});

// ✅ With EngineAPI<GwenServices> — fully typed, no import needed
import type { EngineAPI } from '@gwen/engine-core';
...
> `GwenServices` is declared as a global type in `.gwen/gwen.d.ts` by `gwen prepare`.
> Run `gwen prepare` (or `gwen dev` / `gwen build`, which call it automatically) once
> after adding or removing plugins — then `GwenServices` is up to date everywhere.

> This works identically for Form 1 and Form 2. The annotation goes on each method that uses `api`:
> ```typescript
...
`gwen prepare` reads your `gwen.config.ts`, extracts the plugins you declared in `tsPlugins`, and writes `.gwen/gwen.d.ts`:
...
declare global {
  ...
  // → { keyboard: KeyboardInput; audio: AudioManager; renderer: Canvas2DRenderer; … }
}
```

You never need to export or import `GwenServices` yourself.
