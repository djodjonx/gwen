...
### The `<Services>` generic

`defineUI` accepts an optional generic `<Services>` that types `api.services.get()` inside your rendering methods. **Without it, `api.services.get(...)` returns `unknown`.**

```typescript
// ❌ Without generic — renderer type is unknown
export const PlayerUI = defineUI({
  name: 'PlayerUI',
  render(api, id) {
    const r = api.services.get('renderer'); // unknown — no autocomplete
    r.ctx.fillRect(...); // TS error
  }
});

// ✅ With GwenServices — fully typed, no import needed
...
**`GwenServices` is a global type** — automatically generated by `gwen prepare` into `.gwen/gwen.d.ts`, inferred from your `gwen.config.ts`. No import required.

You only need the generic if you call `api.services` inside your UI — for components that don't touch services, you can omit it entirely.
