...
`onEnter`, `onExit` and `onUpdate` all receive an `api: EngineAPI` parameter. By default, `api.services.get(...)` returns `unknown`. To get full autocompletion, annotate `api` with `EngineAPI<GwenServices>`.
...
// ✅ With EngineAPI<GwenServices> — no import needed
...
> `GwenServices` is declared as a global type in `.gwen/gwen.d.ts` by `gwen prepare`, inferred from the `tsPlugins` in your `gwen.config.ts`. Run `gwen prepare` once after adding or removing plugins (called automatically by `gwen dev` and `gwen build`).

> The same `GwenServices` type is shared across `defineSystem`, `defineScene`, `defineUI` and `definePrefab` — it's always up to date, always global, no imports to maintain.
