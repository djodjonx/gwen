...
> The service types are automatically inferred from `plugins`.
...
The generic parameter `Services` is automatically inferred from the declared `plugins`, enabling type-safe `api.services.get()` calls everywhere.
...
It is inferred from the `plugins` declared in your `gwen.config.ts` and written into `.gwen/gwen.d.ts` — **no export, no import required anywhere in your project**.
...
export const gwenConfig = defineConfig({
  plugins: [new InputPlugin(), new AudioPlugin({ masterVolume: 0.8 })],
});
```
...
import type { GwenConfigServices } from '@djodjonx/gwen-engine-core';
...
| `defineConfig` | ✅ `{ engine, plugins }` | ✗ not applicable | — | ✗ inferred from plugins |
