import type { GwenModuleContext } from '@gwenjs/cli/setup';
import { INPUT_PLUGIN_NAME } from '@gwenjs/input';
 ...
 *
 * Checks:
 * 1. @gwenjs/physics2d is in config.plugins[] (error → stops build)
...
export async function setup(ctx: GwenModuleContext): Promise<void> {
  ...
  if (!hasPhysics) {
    ctx.logger.error(
      '[gwen-kit-platformer] @gwenjs/physics2d is missing from plugins[] in gwen.config.ts.',
    );
  }
