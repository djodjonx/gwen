import type { GwenModuleContext } from '@gwenengine/cli/setup';
import { INPUT_PLUGIN_NAME } from '@gwenengine/input';
 ...
 *
 * Checks:
 * 1. @gwenengine/physics2d is in config.plugins[] (error → stops build)
...
export async function setup(ctx: GwenModuleContext): Promise<void> {
  ...
  if (!hasPhysics) {
    ctx.logger.error(
      '[gwen-kit-platformer] @gwenengine/physics2d is missing from plugins[] in gwen.config.ts.',
    );
  }
