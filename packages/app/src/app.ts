 ...
 *
 * @internal — not part of the public API surface. Use `@gwenengine/app` exports.
 */

import type { GwenPlugin } from '@gwenengine/kit';
import type {
  GwenModule,
  GwenKit,
  AutoImport,
  ...
  private _createKit(config: ResolvedGwenConfig): GwenKit {
    ...
    return {
      ...
      hook<H extends keyof GwenBuildHooks>(event: H, fn: GwenBuildHooks[H]): void {
        // Type assertion is required here because TypeScript cannot resolve
        // `InferCallback<GwenBuildHooks, H>` in a generic context, even though
        // all values in GwenBuildHooks are HookCallback-compatible function types.
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        app.buildHooks.hook(event, fn as any);
      },
...
async function loadModule(name: string): Promise<GwenModule> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const imported: Record<string, any> = await import(name);
...
// Virtual module type declarations for @gwenengine/* virtual imports.
