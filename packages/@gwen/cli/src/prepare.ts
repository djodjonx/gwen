...
export async function prepare(options: PrepareOptions = {}): Promise<PrepareResult> {
  ...
  const hasScenesDir = fs.existsSync(scenesDir);
  if (hasScenesDir) {
  ...
  // ── 4. Générer .gwen/main.ts — point d'entrée auto-géré ────────────────────
  const mainPath = path.join(gwenDir, 'main.ts');
  const mainContent = generateMain(hasScenesDir);
  fs.writeFileSync(mainPath, mainContent, 'utf-8');
  result.files.push(mainPath);
  log(`[gwen prepare] ✅ ${path.relative(projectDir, mainPath)}`);

  // ── 5. S'assurer que le tsconfig.json du projet étend .gwen/ ───────────────
  ensureProjectTsconfig(projectDir, gwenDir, verbose);

  // ── 6. Ajouter .gwen/ dans .gitignore ──────────────────────────────────────
...
// ── Génération de .gwen/main.ts ───────────────────────────────────────────────

/**
 * Génère le point d'entrée principal .gwen/main.ts.
 *
 * Si src/scenes/ existe : importe registerScenes + mainScene depuis ./scenes.ts.
 * Sinon : lance l'engine sans scènes (l'utilisateur les gère dans son code).
 */
function generateMain(hasScenesDir: boolean): string {
  const lines = [
    '/**',
    ' * GWEN - Entry point (auto-generated)',
  ...
  if (hasScenesDir) {
    lines.push("import { registerScenes, mainScene } from './scenes';");
  }
