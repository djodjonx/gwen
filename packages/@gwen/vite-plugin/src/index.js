...
function scanScenes(projectRoot) {
    ...
    return fs
        ...
        const isFactory = !!constMatch && /defineScene\s*\(\s*['"`][^'"`,]+['"`]\s*,/.test(source) && !classMatch;
        // defineScene forme 1 = export const + defineScene({ ... })
        const isConst = !!constMatch && /defineScene\s*\(\s*\{/.test(source) && !classMatch;
        const sceneName = source.match(/defineScene\s*\(\s*['"]([^'"]+)['"]/)?.[1] ??
...
function resolveMainScene(scenes, fromConfig) {
    if (fromConfig)
        return fromConfig;
...
function generateScenesModule(scenes, mainScene) {
    if (scenes.length === 0) {
        return [
...
function generateEntryModule(hasScenesDir) {
    const lines = [
    ...
    lines.push('', 'async function bootstrap() {', '  await initWasm();', hasScenesDir
        ? '  const { engine } = createEngine(gwenConfig, registerScenes, mainScene);'
        : '  const { engine } = createEngine(gwenConfig);', '  engine.start();', '}', '', 'bootstrap().catch(err => {', '  console.error("[GWEN] Fatal:", err);', '  document.body.innerHTML = `<pre style="color:red;padding:2rem">[GWEN] Fatal:\\n${err}</pre>`;', '});');
    return lines.join('\n');
}
// ── Plugin principal ──────────────────────────────────────────────────────────
export function gwen(options = {}) {
    const { wasmPublicPath = '/wasm', wasmMode = 'debug', verbose = false, manifestPath } = options;
    let projectRoot = process.cwd();
    let cratePath = options.cratePath ?? null;
    ...
    function log(msg) {
        if (verbose)
            console.log(`[gwen-vite] ${msg}`);
    }
    function resolveCratePath(root) {
        if (cratePath)
            return path.resolve(root, cratePath);
     ...
     */
    function findPrecompiledWasmDir(root) {
        const candidates = [
    ...
    function startWatcher(root) {
        ...
        fs.watch(srcDir, { recursive: true }, (event, filename) => {
            ...
            if (watchProcess?.exitCode === null)
                return;
            // Compiler dans .gwen/wasm/ (pas dans public/)
            const outDir = path.resolve(root, '.gwen', 'wasm');
            watchProcess = spawn(wasmPack, ['build', '--target', 'web', '--out-dir', outDir, '--dev', crate], { stdio: 'pipe' });
    ...
    return {
        ...
        transformIndexHtml(html) {
            ...
            if (html.includes('/@gwen/entry'))
                return html;
            return html.replace('</body>', '  <script type="module" src="/@gwen/entry"></script>\n</body>');
        },
        ...
        generateBundle() {
            ...
            if (srcDir) {
                ...
                if (files.length > 0)
                    log(`Emitted ${files.length} WASM assets to dist/wasm/`);
            }
            else {
