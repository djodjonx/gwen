 ...
 * is driven entirely by {@link KinematicBodyHandle2D.setVelocity} +
 * `onBeforeUpdate` integration, or by explicit {@link KinematicBodyHandle2D.moveTo} calls.
 */
import { onBeforeUpdate } from '@gwenjs/core';
import { _getActorEntityId } from '@gwenjs/core/scene';
import type { EntityId } from '@gwenjs/core';
import type { KinematicBodyOptions2D, KinematicBodyHandle2D } from '../types';
 ...
 * @returns A {@link KinematicBodyHandle2D} for controlling position and velocity.
 ...
 * @since 1.0.0
 */
export function useKinematicBody(options: KinematicBodyOptions2D = {}): KinematicBodyHandle2D {
  ...
  return {
    ...
    moveTo(x, y, angle = 0) {
      if (!_active) return;
      physics.setKinematicPositionWithAngle(entityId, x, y, angle);
    },
