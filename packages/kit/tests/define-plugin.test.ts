/**
 * @gwenengine/kit — definePlugin() unit tests.
 */
...
describe('definePlugin() — TS-only plugin', () => {
  it('returns a class constructor', () => {
    const MyPlugin = definePlugin(() => ({ name: 'MyPlugin' }));
  ...
  it('calls onInit with the API', () => {
    const onInit = vi.fn();
    const P = definePlugin(() => ({ name: 'P', onInit }));
    const api = mockAPI();
    new P().onInit!(api);
  ...
  it('calls onBeforeUpdate with api and dt', () => {
    ...
    const P = definePlugin(() => ({ name: 'P', onBeforeUpdate }));
    const api = mockAPI();
  ...
  it('calls onUpdate with api and dt', () => {
    ...
    const P = definePlugin(() => ({ name: 'P', onUpdate }));
    const api = mockAPI();
  ...
  it('calls onRender with api', () => {
    ...
    const P = definePlugin(() => ({ name: 'P', onRender }));
  ...
  it('calls onDestroy', () => {
    ...
    const P = definePlugin(() => ({ name: 'P', onDestroy }));
    new P().onDestroy!();
  ...
  it('options are passed to factory()', () => {
...
describe('definePlugin() — WASM plugin', () => {
  function makeWasmPlugin() {
    return definePlugin((_opts: { gravity?: number } = {}) => {
      let stepped = 0;
      return {
        name: 'Physics2D',
  ...
  it('wasm.onInit delegates to onWasmInit()', async () => {
    const P = makeWasmPlugin();
    const p = new P() as GwenPlugin;
    const api = mockAPI();
  ...
  it('wasm.onStep delegates to onStep()', () => {
    const P = makeWasmPlugin();
    const p = new P() as GwenPlugin;
  ...
  it('TS lifecycle is also available on WASM plugin', () => {
    const onUpdate = vi.fn();
    const P = definePlugin(() => ({
      name: 'Hybrid',
      wasm: { id: 'hybrid' },
