...
import { getWasmBridge, createEntityId, unpackEntityId } from '@djodjonx/gwen-engine-core';
...
export const pluginMeta: GwenPluginMeta = {
  serviceTypes: {
    physics3d: {
      from: '@djodjonx/gwen-plugin-physics3d',
...
export const Physics3DPlugin = definePlugin((config: Physics3DConfig = {}) => {
  ...
  const parseBodyState = (arr: Float32Array): Physics3DBodyState => ({
    ...
    rotation: { x: arr[3], y: arr[4], z: arr[5], w: arr[6] },
    linearVelocity: { x: arr[7], y: arr[8], z: arr[9] },
    angularVelocity: { x: arr[10], y: arr[11], z: arr[12] },
  });
