/**
 * Core variant detector for @djodjonx/gwen-cli.
 *
...
import type { GwenConfig } from '@djodjonx/gwen-kit';
 ...
 *
 * Searches for @djodjonx/gwen-engine-core in node_modules and returns
...
export function resolveWasmPath(variant: CoreVariant): {
  ...
  const candidates = [
    ...
    resolve(__dirname, '../../node_modules/@djodjonx/gwen-engine-core'),
    resolve(__dirname, '../../../node_modules/@djodjonx/gwen-engine-core'),
    // Case 2: Running from project root
    resolve(process.cwd(), 'node_modules/@djodjonx/gwen-engine-core'),
  ...
  if (!pkgDir) {
    throw new Error(
      '[GWEN] Could not find @djodjonx/gwen-engine-core package. Please ensure it is installed.',
    );
  }
