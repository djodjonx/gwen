...
export async function buildViteConfig(
  ...
  if (!gwenPlugin) {
    console.warn(
      '[gwen-cli] WARNING: Could not find @djodjonx/gwen-vite-plugin. Virtual modules and WASM serving may not work.',
    );
  }
...
async function loadGwenVitePlugin(projectDir: string): Promise<Function | null> {
  ...
  try {
    ...
    const mod = (await import('@djodjonx/gwen-vite-plugin')) as any;
